...
export const Feed = (): JSX.Element => {
  const { feed, isLoading, isError, hasMore, loadMore, refresh } = useInfiniteFeed({
    limit: 5,
  });
  ...
  /**
   * フィードアイテムをレンダリングする関数
   * @param {FeedItem} item - 表示するフィードアイテム
   * @returns {JSX.Element} 投稿カードコンポーネント
   */
  const renderItem = (item: FeedItem): JSX.Element => (
    <article
      key={item.id}
  ...
  return (
    <div className="space-y-4">
      ...
      {feed.length === 0 && !isLoading ? (
        ...
        <div className="space-y-2">{feed.map(renderItem)}</div>
      )}
